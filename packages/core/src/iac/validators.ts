...
export const v = {
	...
	id: <T extends string>(tableName: T) => z.string().brand<`${T}Id`>(),
	/** ISO 8601 datetime string */
	datetime: () => z.string().datetime({ offset: true }),
	/** Bytes (base64 string) */
	bytes: () => z.string().base64(),
	/** Full-text search field — creates a tsvector in PostgreSQL */
	fullText: () => {
		const schema = z.string();
		// Attach metadata for migration generator
		(schema as any)._isFullText = true;
		return schema;
	},
	/** Vector field for similarity search — requires pgvector extension
	 * @param dimensions - Number of dimensions (e.g., 1536 for OpenAI embeddings) */
	vector: (dimensions: number) => {
		...
		const schema = z.array(z.number()).length(dimensions);
		// Attach metadata for migration generator
		(schema as any)._isVector = true;
		(schema as any)._dimensions = dimensions;
		return schema;
	},
};
