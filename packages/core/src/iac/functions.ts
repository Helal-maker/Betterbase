...
export interface QueryRegistration<TArgs extends z.ZodRawShape, TReturn> {
	[FUNCTION_KIND]: "query";
	_args: z.ZodObject<TArgs>;
	_handler: (ctx: QueryCtx, args: z.infer<z.ZodObject<TArgs>>) => Promise<TReturn>;
}

export interface MutationRegistration<TArgs extends z.ZodRawShape, TReturn> {
	[FUNCTION_KIND]: "mutation";
	_args: z.ZodObject<TArgs>;
	_handler: (ctx: MutationCtx, args: z.infer<z.ZodObject<TArgs>>) => Promise<TReturn>;
	/** Optional function that returns the shape of data the client should display immediately */
	_optimistic?: (args: z.infer<z.ZodObject<TArgs>>) => unknown;
}

export interface ActionRegistration<TArgs extends z.ZodRawShape, TReturn> {
...
export function mutation<TArgs extends z.ZodRawShape, TReturn>(config: {
	...
	/** Optional function that returns the shape of data the client should display immediately.
	 * This enables optimistic updates - the UI updates before the server confirms. */
	optimistic?: (args: z.infer<z.ZodObject<TArgs>>) => unknown;
}): MutationRegistration<TArgs, TReturn> {
	return {
		[FUNCTION_KIND]: "mutation",
		_args: z.object(config.args),
		_handler: config.handler,
		_optimistic: config.optimistic,
	};
}
